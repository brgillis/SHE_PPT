...
def convert_to_hdf5(det_file, bkg_file, wgt_file, seg_file, output_filename, chunk=None):
    ...
    Inputs:
      ...
      - chunk: [optional] whether to chunk the file or not. If chunking is requested, this should be a tuple of integers denoting the chunk size
    ...
    for name, sci, rms, flg, wgt, bkg, seg, wcs in zip(
        ...
        sci_ds = create_dataset_from_hdu(g, "sci", sci, chunk)
        rms_ds = create_dataset_from_hdu(g, "rms", rms, chunk)
        flg_ds = create_dataset_from_hdu(g, "flg", flg, chunk)
        wgt_ds = create_dataset_from_hdu(g, "wgt", wgt, chunk)
        bkg_ds = create_dataset_from_hdu(g, "bkg", bkg, chunk)
        seg_ds = create_dataset_from_hdu(g, "seg", seg, chunk)
