...
def __create_fits(object_ids, pixel_coords, workdir, stampsize, stamp_per_obj):
    ...
    if stamp_per_obj:
        for obj_id in object_ids:
            bpsf_hdu = fits.ImageHDU(img,name="%d.BPSF"%obj_id, header=fits.Header({"GS_SCALE":5.55555555555555E-06}))
            dpsf_hdu = fits.ImageHDU(img,name="%d.DPSF"%obj_id, header=fits.Header({"GS_SCALE":5.55555555555555E-06}))
